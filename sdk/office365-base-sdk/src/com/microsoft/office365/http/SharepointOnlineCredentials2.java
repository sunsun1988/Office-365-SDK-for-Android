...
import com.microsoft.office365.Platform2;

@SuppressLint("SetJavaScriptEnabled")
public class SharepointOnlineCredentials2 extends OAuthCredentials {
	...
	public SharepointOnlineCredentials2(String oAuthToken, String refreshToken) {
		super(oAuthToken);
		mToken = oAuthToken;
	...
	public static OfficeFuture<SharepointOnlineCredentials2> requestCredentials(
			Activity activity, String siteUrl, final String clientId,
			final String redirectUrl, final String office365Domain,
			final String clientSecret, final String refreshToken)
			throws MalformedURLException {

		final OfficeFuture<SharepointOnlineCredentials2> credentialsFuture = new OfficeFuture<SharepointOnlineCredentials2>();

		URL url = new URL(siteUrl);
		final String sharepointHost = url.getHost();

		String authCodeUrl = String.format(
				AUTHORIZATION_CODE_REQUEST_URL_FORMAT, siteUrl, clientId,
				redirectUrl);

		OfficeFuture<String> codeFuture;

		if (refreshToken == null) {
			codeFuture = showLoginForAcccessCode(activity, authCodeUrl,
		...
		codeFuture.done(new Action<String>() {
			...
			public void run(String accessCode) throws Exception {
				...
				String accessTokenRequestUrl = String.format(
						ACCESS_TOKEN_REQUEST_URL_FORMAT, office365Domain);
				...
				if (refreshToken == null) {
					requestContent = String
							.format(ACCESS_TOKEN_REQUEST_CONTENT_WITH_AUTHORIZATION_CODE,
									encode(clientId), office365Domain,
									encode(clientSecret), accessCode,
									redirectUrl, sharepointHost,
									office365Domain);
				} else {
					requestContent = String.format(
							ACCESS_TOKEN_REQUEST_CONTENT_WITH_REFRESH_TOKEN,
							encode(clientId), office365Domain,
							encode(clientSecret), refreshToken, redirectUrl,
							sharepointHost, office365Domain);
				}

				get.setContent(requestContent);

				final HttpConnectionFuture2 accessTokenFuture = connection
						.execute(get);

				accessTokenFuture.addListener(new Runnable() {
					...
					public void run() {
						try {
							...
							credentialsFuture.triggerError(t.getCause());
						} catch (Throwable t) {
						}
					}
				...
				accessTokenFuture.addListener(new Runnable() {
					...
					public void run() {
						try {
							Response accessTokenResponse = accessTokenFuture
									.get();
							...
							String accessToken = json
									.getString(ACCESS_TOKEN_NODE);

							String newRefreshToken;
							if (json.has(REFRESH_TOKEN_NODE)) {
								newRefreshToken = json
										.getString(REFRESH_TOKEN_NODE);
							} else {
								newRefreshToken = refreshToken;
							}

							credentialsFuture
									.setResult(new SharepointOnlineCredentials2(
	...
	protected static OfficeFuture<String> showLoginForAcccessCode(
			Activity activity, final String startUrl, String clientId,
			final String endUrl) {
		...
		wv.setLayoutParams(new ViewGroup.LayoutParams(
				ViewGroup.LayoutParams.MATCH_PARENT, webViewHeight));
		...
		wv.setWebViewClient(new WebViewClient() {
			 ...
			 * private boolean isStartUrl(String url) { if (url == null) {
			 * return false; }
			 * 
			 * return url.startsWith(startUrl); }
			 * 
			 * @Override public void onPageFinished(WebView view, String url) {
			 * if (isStartUrl(url)) { if (externalCallback != null) {
